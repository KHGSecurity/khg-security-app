// Looks up a ServiceM8 job by its job number, then adds a Note to it containing
// a submitted form's content. The ServiceM8 API key lives only here, as a
// Netlify environment variable (SERVICEM8_API_KEY) — it never reaches the browser.
...
exports.handler = async function (event) {
  ...
  if (!jobNumber || !note) {
    return { statusCode: 400, body: JSON.stringify({ success: false, error: 'Missing job number or note content.' }) };
  }
  ...
  try {
    ...
    if (!noteRes.ok) {
      ...
      return { statusCode: 502, body: JSON.stringify({ success: false, error: 'ServiceM8 rejected the note.', detail }) };
    }

    return { statusCode: 200, body: JSON.stringify({ success: true }) };
  } catch (err) {
    console.error('ServiceM8 note function error:', err);
    return {
      statusCode: 500,
      body: JSON.stringify({ success: false, error: (err && err.message) || 'Unknown error contacting ServiceM8.', detail: (err && err.stack) || String(err) })
    };
  }
};
